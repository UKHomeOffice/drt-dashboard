...
class QueueExportTest extends AnyWordSpec with Matchers {
  ...
  "QueueExport" should {
    "return a PortQueuesJson with the correct structure and only the values in the requested time range" in {
      ...
      val source: (PortCode, Terminal, UtcDate, UtcDate) => Source[CrunchMinute, NotUsed] =
        (_: PortCode, _: Terminal, _: UtcDate, _: UtcDate) => {
          Source(List(
            CrunchMinute(
              T1,
              EeaDesk,
              start.addMinutes(-15).millisSinceEpoch,
              10d,
              0d,
              0,
              0,
