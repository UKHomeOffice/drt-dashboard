...
import uk.gov.homeoffice.drt.routes.api.v1_1.FlightApiV1_1Routes.{
  FlightJsonResponseV1_1,
  FlightJsonV1_1,
  FlightQueuePaxCountJsonV1_1
}
      ...
      val source = (_: PortCode, _: List[FeedSource], _: LocalDate, _: LocalDate, _: Seq[Terminal]) => {
        Source(
          Seq(
            ...
            ArrivalGenerator.arrival(
              iata = "BA0002",
              schDt = sched1.toISOString,
              estDt = sched1.addMinutes(1).toISOString,
              actChoxDt = sched1.addMinutes(5).toISOString,
              totalPax = Option(100),
              transPax = Option(10),
              feedSource = LiveFeedSource
            ),
        ...
        FlightJsonResponseV1_1(
          ...
          Seq(
            FlightJsonV1_1(
              ...
              "JFK",
              "John F Kennedy Intl",
              sched1.millisSinceEpoch,
              Option(sched1.addMinutes(1).millisSinceEpoch),
              Option(sched1.addMinutes(5).millisSinceEpoch),
              Some(sched1.addMinutes(5).millisSinceEpoch),
              Some(sched1.addMinutes(9).millisSinceEpoch),
              Some(90),
              "On Chocks",
              Some(Seq(FlightQueuePaxCountJsonV1_1("e-Gates", 67), FlightQueuePaxCountJsonV1_1("EEA", 23)))
            ),
            FlightJsonV1_1(
              "STN",
              "T1",
              ...
              "JFK",
              "John F Kennedy Intl",
              sched2.millisSinceEpoch,
              None,
              None,
              Some(sched2.addMinutes(5).millisSinceEpoch),
              Some(sched2.addMinutes(14).millisSinceEpoch),
              Some(190),
              "Scheduled",
              Some(Seq(FlightQueuePaxCountJsonV1_1("e-Gates", 142), FlightQueuePaxCountJsonV1_1("EEA", 48)))
            )
          )
        )
      )
