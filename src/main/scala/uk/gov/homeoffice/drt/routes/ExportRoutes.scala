...
object ExportRoutes {
  ...
  def apply(
    ...
    val flightsWithManifestsForPortDatesSource
        : (PortCode, LocalDate, LocalDate) => Source[(Seq[ApiFlightWithSplits], VoyageManifests), NotUsed] = {
          ...
          pathEnd(
            post(entity(as[ExportRequest]) { exportRequest =>
              handleExport(
                ...
                teamEmail,
                rootUrl,
                flightsWithManifestsForPortDatesSource,
                feedSourceOrder
              )
            })
          ),
  ...
  private def handleExport(
      ...
      flightsWithManifestsForPortDatesSource: (
          ...
          LocalDate,
          LocalDate
      ) => Source[(Seq[ApiFlightWithSplits], VoyageManifests), NotUsed],
      feedSourceOrder: PortCode => List[FeedSource]
    ...
    val export = Export(
      email,
      exportRequest.ports.map(ep => ep.terminals.map(t => s"${ep.port}-$t").mkString("_")).mkString("__"),
      ...
      case Arrivals =>
        Source(exportRequest.ports.toList.sortBy(_.port))
          .flatMap { exportPort =>
            ...
            val terminals = AirportConfigs.confByPort.get(portCode).map(_.terminalsForDateRange(
              exportRequest.startDate,
              exportRequest.endDate
            )).getOrElse(Seq.empty).toSeq
            val fwsExport = FlightsWithSplitsMultiRegionExportImpl(
              exportRequest.startDate,
              exportRequest.endDate,
              portCode,
              terminals,
            ...
            val flightsWithManifestsSource =
              flightsWithManifestsForPortDatesSource(portCode, exportRequest.startDate, exportRequest.endDate)
    ...
    complete(s"""{"status": "${export.status}", "createdAt": ${export.createdAt.millisSinceEpoch}, "downloadLink": "${downloadUrl(
        rootUrl,
        export
      )}"}""")
  }

  private def restExportStream(exportRequest: ExportRequest, exportCsvService: RestExportCsvService)(implicit
      ec: ExecutionContext,
      mat: Materializer
  ...
  private def addLiveManifestsForFlights(
      portCode: PortCode,
  ...
  private def handleReportReady(
    ...
    val emailSuccess =
      emailClient.send(DownloadManagerTemplates.reportReadyTemplateId, export.email, Map("download_link" -> link))

    if (!emailSuccess) log.error("Failed to send email")
  }

  private def downloadUrl(rootDomain: String, export: Export): String =
  ...
  private def handleReportFailure(
    ...
    val emailSuccess =
      emailClient.send(DownloadManagerTemplates.reportFailedTemplateId, export.email, Map("support_email" -> teamEmail))
