...
object ExportRoutes {
  ...
  def apply(httpClient: HttpClient,
            ...
            rootUrl: String,
            teamEmail: String,
            paxSummaryProvider: (LocalDate, LocalDate, Granularity, Option[Terminal]) => PortCode => Source[(Map[Queue, Int], Int, Option[Any]), NotUsed],
           )
        ...
        concat(
          pathEnd(
            post(entity(as[ExportRequest]) { exportRequest =>
              handleExport(upload, exportPersistence, exportCsvService, email, exportRequest, now, emailClient, rootUrl, teamEmail, rootUrl, paxSummaryProvider)
            })
          ),
          get {
            concat(
  ...
  private def handleExport(upload: (String, Source[ByteString, Any]) => Future[Done],
    ...
    val stream = if (exportRequest.exportType == Arrivals) {
      ...
      val eventualString = PassengerRoutes
        .passengerContent(
          portCodes = exportRequest.ports.map(_.port),
          maybeTerminal = None,
          summaryProvider = paxSummaryProvider,
          start = exportRequest.startDate,
          end = exportRequest.endDate,
          granularity = Granularity.fromString(exportRequest.exportType.toString),
          contentType = ContentTypes.`text/csv(UTF-8)`,
        )
        .map(ByteString(_))
      Source.future(eventualString)
    }
