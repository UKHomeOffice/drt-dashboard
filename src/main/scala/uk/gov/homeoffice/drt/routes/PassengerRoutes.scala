...
object PassengerRoutes {
  ...
  def apply(summaryProvider: (
      LocalDate,
      LocalDate,
      Granularity,
      Option[Terminal]
  ) => PortCode => Source[(Map[Queue, Int], Int, Map[Queue, Int], Option[Any]), NotUsed])(implicit
      ec: ExecutionContext,
      mat: Materializer
  ): Route =
  ...
  private def passengersForPort(
      portCodes: Iterable[String],
      startDate: String,
      endDate: String,
      ...
      summaryProvider: (
          ...
          Granularity,
          Option[Terminal]
      ) => PortCode => Source[(Map[Queue, Int], Int, Map[Queue, Int], Option[Any]), NotUsed]
  ...
  private def sourceToContent(
    ...
    if (contentType == ContentTypes.`text/csv(UTF-8)`)
      portResult.runFold("") {
        case (acc, (portCode, (queues, capacity, bx, maybeDateOrDateHour))) => acc +
            passengersCsvRow(portCode, maybeTerminal, queues, capacity, bx, maybeDateOrDateHour)
      }
    else {
      import uk.gov.homeoffice.drt.jsonformats.PassengersSummaryFormat._
      portResult
        .runFold(PassengersSummaries.empty) {
          case (acc, (portCode, (queues, capacity, bx, x))) => acc ++
              Seq(passengersJson(portCode, maybeTerminal, queues, capacity, bx, x))
        }
        .map(_.summaries.toJson.compactPrint)
    }
  }

  private def passengersCsvRow[T]
      : (PortCode, Option[String], Map[Queue, Int], Int, Map[Queue, Int], Option[T]) => String =
    (portCode, maybeTerminal, drtQueueCounts, capacity, bxQueueCounts, maybeDateOrDateHour) => {
      ...
      maybeTerminal match {
        case Some(terminal) =>
          (dateStr.toList ++
            List(regionName, portCodeStr, terminal, capacity, totalPcpPax, drtQueueCells, bxQueueCells)).mkString(",") +
            "\n"
        case None =>
          (dateStr.toList ++
            List(regionName, portCodeStr, capacity, totalPcpPax, drtQueueCells, bxQueueCells)).mkString(",") + "\n"
      }
    }

  private def passengersJson[T]
      : (PortCode, Option[String], Map[Queue, Int], Int, Map[Queue, Int], Option[T]) => PassengersSummary =
    (portCode, maybeTerminal, drtQueueCounts, capacity, bxQueueCounts, maybeDateOrDateHour) => {
      val regionName = PortRegion.fromPort(portCode).name
      val portCodeStr = portCode.toString
