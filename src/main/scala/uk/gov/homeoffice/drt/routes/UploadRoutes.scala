...
object UploadRoutes {
  ...
  def apply(prefix: String, urls: Urls, neboPortCodes: List[String])(implicit ec: ExecutionContextExecutor, mat: Materializer): Route = {
    val route: Route =
      pathPrefix(prefix) {
        post {
          fileUpload("csv") {
            case (metadata, byteSource) =>
              ...
              val responseF = neboPortCodes.map { portCode =>
                flightData.flatMap(r => sendData(r.filter(_.flightCode.toLowerCase == portCode.toLowerCase), s"${urls.urlForPort(portCode)}$routePath"))
              }
  ...
  def rowToJson(rows: List[Row], metadata: FileInfo) = {
