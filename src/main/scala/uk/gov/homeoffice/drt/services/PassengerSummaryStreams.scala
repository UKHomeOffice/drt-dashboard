...
case class PassengerSummaryStreams(db: AppDatabase)(implicit ec: ExecutionContext) {
  val streamForGranularity: (LocalDate, LocalDate, Granularity, Option[Terminal]) => PortCode => Source[
    (Map[Queue, Int], Int, Map[Queue, Int], Option[Any]),
    NotUsed
  ] =
    (start, end, granularity, maybeTerminal) =>
      portCode => {
        ...
        val stream = granularity match {
          case Hourly =>
            val hourlyQueueTotalsQuery =
              PassengersHourlyDao.hourlyForPortAndDate(portCode.iata, maybeTerminal.map(_.toString))
            val hourlyCapacityTotalsQuery =
              CapacityHourlyDao.hourlyForPortAndDate(portCode.iata, maybeTerminal.map(_.toString))
            val hourlyBxTotalsQuery =
              BorderCrossingDao.hourlyForPortAndDate(portCode.iata, maybeTerminal.map(_.toString))
  ...
  private val hourlyStream: (
      LocalDate => Future[Map[Long, Map[Queue, Int]]],
      LocalDate => Future[Map[Long, Int]],
      LocalDate => Future[Map[Long, Map[Queue, Int]]]
  ) => (LocalDate, LocalDate) => Source[(Map[Queue, Int], Int, Map[Queue, Int], Option[Long]), NotUsed] =
    (queueHourlyForDate, capacityHourlyForDate, bxHourlyForDate) =>
      (start, end) =>
  ...
  private val dailyStream: (
      LocalDate => Future[Map[Queue, Int]],
      LocalDate => Future[Int],
      LocalDate => Future[Map[Queue, Int]]
  ) => (LocalDate, LocalDate) => Source[(Map[Queue, Int], Int, Map[Queue, Int], Option[LocalDate]), NotUsed] =
    (queueTotalsForDate, capacityTotalForDate, bxTotalForDate) =>
      (start, end) =>
        Source(DateRange(start, end))
