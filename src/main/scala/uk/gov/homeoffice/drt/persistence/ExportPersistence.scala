...
case class ExportPersistenceImpl(database: AppDatabase)(implicit ec: ExecutionContext) extends ExportPersistence {
  ...
  override def get(email: String, createdAt: Long): Future[Option[Export]] =
    database.db.run(ExportQueries.get(email, createdAt))
