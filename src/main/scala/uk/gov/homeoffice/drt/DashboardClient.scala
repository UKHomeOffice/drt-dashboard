...
object DashboardClient {
  ...
  def getWithRoles(uri: String, roles: Iterable[Role])(implicit
      system: ClassicActorSystemProvider
  ): Future[HttpResponse] =
    Http().singleRequest(Get(uri).withHeaders(rolesToRoleHeader(roles)))

  def postWithRoles(uri: String, json: String, roles: Iterable[Role])(implicit
      system: ClassicActorSystemProvider
  ): Future[HttpResponse] =
    Http().singleRequest(Post(
      uri,
      HttpEntity(ContentTypes.`text/plain(UTF-8)`, json)
    ).withHeaders(rolesToRoleHeader(roles)))

  def deleteWithRoles(uri: String, roles: Iterable[Role])(implicit
      system: ClassicActorSystemProvider
  ): Future[HttpResponse] =
