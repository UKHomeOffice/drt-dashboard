...
class ScheduledHealthCheckPauseTable(tag: Tag)
    extends Table[ScheduledHealthCheckPauseRow](tag, "scheduled_health_check_pause") {
  ...
  def * : ProvenShape[ScheduledHealthCheckPauseRow] =
    (startsAt, endsAt, ports, createdAt).mapTo[ScheduledHealthCheckPauseRow]
...
case class ScheduledHealthCheckPauseDao(now: () => SDateLike) {
  ...
  def get(maybeAfter: Option[Long])(implicit
      ec: ExecutionContext
  ): DBIOAction[Seq[ScheduledPause], NoStream, Effect.Read] = {
