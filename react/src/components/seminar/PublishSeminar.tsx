...
export function PublishSeminar(props: Props) {
    const [error, setError] = useState(false);
    const [confirmAction, setConfirmAction] = useState(false);
    const handleResponse = (response: AxiosResponse) => {
        if (response.status === 200) {
            console.log('Seminar' + props.actionString);
        } else {
            setError(true);
            response.data
        }
    ...
    const confirmPublishFeature = () => {
        axios.post('/seminar/published/' + props.id, {published: props.actionString == "publish"})
        ...
        error ?
            ...
            confirmAction ?
                <Dialog open={confirmAction} onClose={handleCloseConfirmDialog}>
                    <DialogTitle>Seminar Published</DialogTitle>
                    <DialogContent>
                        <p>Seminar requested is {props.actionString == "publish" ? "Publish" : "UnPublish"}</p>
                    </DialogContent>
                    <DialogActions>
